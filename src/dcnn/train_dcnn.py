...
parser = argparse.ArgumentParser(
    description='Train a DCNN on the binary Stanford Sentiment dataset as specified in the Kalchbrenner \'14 paper. All the default values are taken from the paper or the Matlab code.')
# training settings
...
parser.add_argument("--adagrad_reset", type=int, default=5,
                    help="Resets the adagrad cumulative gradient after x epochs. If the value is 0, no reset will be executed.")
# input output
parser.add_argument("--vocab_size", type=int, default=3857, help='Vocabulary size')
parser.add_argument("--output_classes", type=int, default=5, help='Number of output classes')
parser.add_argument("--batch_size", type=int, default=1, help='Batch size')
# network paras
parser.add_argument("--word_vector_size", type=int, default=48, help='Word vector size')
parser.add_argument("--filter_size_conv_layers", nargs="+", type=int, default=[7, 5],
                    help="List of sizes of filters at layer 1 and 2, default=[10,7]")
parser.add_argument("--nr_of_filters_conv_layers", nargs="+", type=int, default=[6, 14],
                    help="List of number of filters at layer 1 and 2, default=[6,12]")
parser.add_argument("--activations", nargs='+', type=str, default=["tanh", "tanh"],
...
parser.add_argument("--L2", nargs='+', type=float, default=[0.0001 / 2, 0.00003 / 2, 0.000003 / 2, 0.0001 / 2],
                    help="Fine-grained L2 regularization. 4 values are needed for 4 layers, namly for the embeddings layer, 2 conv layers and a final/output dense layer.")
...
if len(hyperparas['filter_size_conv_layers']) != 2 or len(hyperparas['nr_of_filters_conv_layers']) != 2 or len(
        hyperparas['activations']) != 2 or len(hyperparas["L2"]) != 4:
    raise Exception(
        'Check if the input --filter_size_conv_layers, --nr_of_filters_conv_layers and --activations are lists of size 2, and the --L2 field needs a value list of 4 values.')
...
loss_train = lasagne.objectives.aggregate(
    ...
    mode='mean') + lasagne.regularization.regularize_layer_params_weighted(dict(zip(l2_layers, hyperparas["L2"])),
        ...
        if batch_counter > 0 and batch_counter % hyperparas["valid_freq"] == 0:
            ...
            for minibatch_dev_index in range(n_dev_batches):
                x_input = dev_x_indexes_extended[
                          ...
                          0:dev_lengths[(minibatch_dev_index + 1) * batch_size - 1]]
            ...
            if this_validation_accuracy > best_validation_accuracy:
                print("Train loss, " + str((train_loss / hyperparas["valid_freq"])) + ", validation accuracy: " + str(
                ...
                for minibatch_test_index in range(n_test_batches):
                    ...
                    print(test_model(x_input, y_input))
