...
class KMaxPoolLayer(Layer):
    ...
    def kmaxpooling(self, input, k):
        ...
        dim1 = T.arange(0, self.input_shape[1]).repeat(k * self.input_shape[2]).reshape((1, -1)).repeat(
            self.input_shape[0], axis=0).flatten()
        dim2 = T.arange(0, self.input_shape[2]).repeat(k).reshape((1, -1)).repeat(
            self.input_shape[0] * self.input_shape[1], axis=0).flatten()
...
class DynamicKMaxPoolLayer(KMaxPoolLayer):
    ...
    def get_k(self, input_shape):
        return T.cast(
            T.max([self.ktop, T.ceil((self.nroflayers - self.layernr) / float(self.nroflayers) * input_shape[3])]),
            'int32')
